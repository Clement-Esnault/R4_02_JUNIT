...
/**
 * Cette classe permet de simuler le calcul de l'impôt sur le revenu en France
 * pour l'année 2024 sur les revenus de l'année 2023 pour des cas simples de
 * contribuables célibataires, mariés, divorcés, veufs ou pacsés avec ou sans
 * enfants à charge ou enfants en situation de handicap et parent isolé.
 *
 * EXEMPLE DE CODE DE TRES MAUVAISE QUALITE FAIT PAR UN DEBUTANT
 *
 * Pas de lisibilité, pas de commentaires, pas de tests Pas de documentation,
 * pas de gestion des erreurs Pas de logique métier, pas de modularité Pas de
 * gestion des exceptions, pas de gestion des logs Principe "Single
 * Responsability" non respecté Pas de traçabilité vers les exigences métier
 *
 * Pourtant ce code fonctionne correctement Il s'agit d'un "legacy" code qui est
 * difficile à maintenir L'auteur n'a pas fourni de tests unitaires
 **/
...
public class Simulateur {
	...
	// Fonction de calcul de l'impôt sur le revenu net en France en 2024 sur les
	// revenu 2023

	public long calculImpot(int revNet, SituationFamiliale sitFam, int nbEnfants, int nbEnfantsHandicapes,
			boolean parentIsol) {
	...
	/*
	 * public static void main(String[] args) { Simulateur simulateur = new
	 * Simulateur(); long impot = simulateur.calculImpot(65000,
	 * SituationFamiliale.MARIE, 3, 0, false);
	 * System.out.println("Impot sur le revenu net : " + impot); impot =
	 * simulateur.calculImpot(65000, SituationFamiliale.MARIE, 3, 1, false);
	 * System.out.println("Impot sur le revenu net : " + impot); impot =
	 * simulateur.calculImpot(35000, SituationFamiliale.DIVORCE, 1, 0, true);
	 * System.out.println("Impot sur le revenu net : " + impot); impot =
	 * simulateur.calculImpot(35000, SituationFamiliale.DIVORCE, 2, 0, true);
	 * System.out.println("Impot sur le revenu net : " + impot); impot =
	 * simulateur.calculImpot(50000, SituationFamiliale.DIVORCE, 3, 0, true);
	 * System.out.println("Impot sur le revenu net : " + impot); impot =
	 * simulateur.calculImpot(50000, SituationFamiliale.DIVORCE, 3, 1, true);
	 * System.out.println("Impot sur le revenu net : " + impot); impot =
	 * simulateur.calculImpot(200000, SituationFamiliale.CELIBATAIRE, 0, 0, true);
	 * System.out.println("Impot sur le revenu net : " + impot);
	 * 
	 * }
	 */

	// Champs nécessaires pour l'adaptation
	private SituationFamiliale sitFam;

	// Setter rajouté par Olivier CHARLES le 13/02/2026 ( Adaptateur )
	...
	public void setNbEnfantsACharge(int nbEnfantsACharge) {
		this.nbEnf = nbEnfantsACharge;
	}

	// Contrainte : nb enfants situation handicap <= nb enfants à charge
	public void setNbEnfantsSituationHandicap(int nbEnfantsSituationHandicap) {
		this.nbEnfH = nbEnfantsSituationHandicap;
	}
	...
	public void calculImpotSurRevenuNet() {
		this.calculImpot(rNet, sitFam, nbEnf, nbEnfH, parIso);
	}
	...
	public int getImpotSurRevenuNet() {
		return (int) Math.round(this.mImp);
	}

	// On rajoute la décote pour obtenir l'impot avant decote
	public int getImpotAvantDecote() {
		return (int) Math.round(this.mImp) + this.getDecote();
	}
}
